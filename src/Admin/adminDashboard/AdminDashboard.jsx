...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    dispatch(GetTotel()).then((res) => {
      setSales(res.payload);
    });
    dispatch(AdminOrder()).then((res) => {
      ...
      {open && (
        <div className="w-64 bg-gradient-to-r from-purple-500 to-indigo-600 text-white flex flex-col">
          ...
          <nav className="flex-1 p-6">
            <a className="block py-2 px-4 rounded hover:bg-indigo-700" onClick={() => setNav('')}>
              Dashboard
            </a>
            <a className="block py-2 px-4 rounded hover:bg-indigo-700" onClick={() => setNav('Users')}>
              Users
            </a>
            <a className="block py-2 px-4 rounded hover:bg-indigo-700" onClick={() => setNav('products')}>
              Products
            </a>
            <a className="block py-2 px-4 rounded hover:bg-indigo-700" onClick={() => setNav('addpro')}>
              Add Products
            </a>
            <a className="block py-2 px-4 rounded hover:bg-indigo-700" onClick={() => setNav('orders')}>
              Orders
            </a>
        ...
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 shadow-md">
          <div className="p-4 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              <button className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-700 shadow" onClick={handleLogout}>
                Logout
              </button>
